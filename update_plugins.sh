echo "Accept plugin"
cd accept
git pull
cd ..

echo "Clipboard image paste"
cd clipboard_image_paste
git pull
cd ..

echo "My Projects plugin"
cd my_projects
git pull
cd ..

echo "Progressive Projects List plugin"
cd progressive_projects_list
git pull
cd ..

echo "Redmine Select2 Plugin"
cd redmine__select2
git pull
cd ..

echo "Redmine Backlogs"
cd redmine_backlogs
git pull
cd ..

echo "Redmine Banner plugin"
cd redmine_banner
git pull
cd ..

echo "Redmine Checklist"
cd redmine_checklists
git pull
cd ..

echo "Redmine Close Button"
cd redmine_close_button
git pull
cd ..

echo "Redmine Emoji Button"
cd redmine_emojibutton
git pull
cd ..

echo "Gamification plugin"
cd redmine_gamification_plugin
git pull
cd ..

echo "Redmine Graphs plugin"
cd redmine-graphs-plugin
git pull
cd ..

echo "Redmine Hotkeys Js plugin"
cd redmine_hotkeys_js
git pull
cd ..

echo "Redmine Issue Badge plugin"
cd redmine_issue_badge
git pull
cd ..

echo "Redmine issue completion"
cd redmine_issue_completion
git pull
cd ..

echo "Redmine Issue Evm plugin"
cd redmine_issue_evm
git pull
cd ..

echo "Redmine Issue Favicon plugin"
cd redmine_issue_favicon
git pull
cd ..

echo "Redmine Issues Tree plugin"
cd redmine_issues_tree
git pull
cd ..

echo "Redmine jsToolbar Extension"
cd redmine_jstoolbar_ext
git pull
cd ..

echo "Redmine jsToolbar Buttons Extension"
cd redmine_jstoolbar_ext_buttons
git pull
cd ..

echo "Redmine Lightbox 2"
cd redmine_lightbox2
git pull
cd ..

echo "Redmine Login Audit plugin"
cd redmine_login_audit
git pull
cd ..

echo "Redmine (Monitoring & Controlling | Monitoramento & Controle)"
cd redmine_monitoring_controlling
git pull
cd ..

echo "Redmine Pivot Table plugin"
cd redmine_pivot_table
git pull
cd ..

echo "Redmine Q&A Plugin"
cd redmine_questions
git pull
cd ..

echo "Redmine Agile"
cd redmine_agile
git pull
cd ..

echo "Redmine People"
cd redmine_people
git pull
cd ..

echo "Revision Branches"
cd redmine_revision_branches
git pull
cd ..

echo "Redmine Scrum Cards plugin"
cd redmine_scrum_cards
git pull
cd ..

echo "Redmine Spent Time plugin"
cd redmine_spent_time
git pull
cd ..

echo "Redmine Sticky Messages plugin"
cd redmine_sticky_messages
git pull
cd ..

echo "Tab Plugin"
cd redmine_tab
git pull
cd ..

echo "Redmine Tags"
cd redmine_tags
git pull
cd ..

echo "Redmine Spent Time plugin"
cd redmine_timesheet_plugin
git pull
cd ..

echo "Redmine Workflow Enhancements"
cd redmine_workflow_enhancements
git pull
cd ..

echo "Redmine Zen Edit plugin"
cd redmine_zenedit
git pull
cd ..

echo "Redrisk plugin"
cd redrisk
git pull
cd ..

echo "Release Logs plugin"
cd release_logs
git pull
cd ..

echo "Sidebar Hide Plugin"
cd sidebar_hide
git pull
cd ..

echo "Simple Author Change plugin"
cd simple_author_change
git pull
cd ..

echo "Timelog Timer plugin"
cd timelog_timer
git pull
cd ..